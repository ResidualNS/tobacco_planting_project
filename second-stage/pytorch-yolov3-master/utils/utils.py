...
def non_max_suppression(prediction, conf_thres=0.3, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    """
    ...
    # Box constraints
    min_wh, max_wh = 10, 200  # (pixels) minimum and maximum box width and height
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        r=((x[:,2:3]/x[:,3:4]>0.4) & (x[:,2:3]/x[:,3:4]<2.5)).all(1)
        if len(r) != 0:
            x=x[r]
