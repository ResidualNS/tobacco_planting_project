...
hyp = {'giou': 3.54,  # giou loss gain
       ...
       'cls_pw': 0.1,  # cls BCELoss positive_weight
       'obj': 64.3,  # obj loss gain (*=img_size/320 if img_size != 320)
       'obj_pw': 1.0,  # obj BCELoss positive_weight
       'iou_t': 0.20,  # iou training threshold
       'lr0': 0.0001,  # initial learning rate (SGD=5E-3, Adam=5E-4)
       'lrf': 0.0001,  # final learning rate (with cos scheduler)0.0005
...
def train(hyp):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            ...
            results, maps = test.evaluate(cfg,
                                      data,
                                      batch_size=batch_size,
                                      img_size=imgsz_test,
                                      model=ema.ema,
        ...
        if save:
            ...
            if (best_fitness == fi) and not final_epoch:
                torch.save(chkpt, best+'%d.pt'% (epoch))
            elif epoch % opt.checkpoint_interval == 0:
                torch.save(chkpt, wdir+'yolov3-tiny_chkpt_%d.pt' % epoch)
            del chkpt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/yzy.data', help='*.data path')
    parser.add_argument('--multi-scale',  default=True, help='adjust (67%% - 150%%) img_size every 10 batches')
    ...
    if not opt.evolve:  # Train normally
        print('Start Tensorboard with "tensorboard --logdir=runs --host=127.0.0.1", view at http://localhost:6006/')
