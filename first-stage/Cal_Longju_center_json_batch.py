...
class Cal_Longju():
    ...
    def error_points(self, Center_Points, Longju, xiangyuan):
        NormalCenter_Points = []
        NormalLongju = []
        for i in range(len(Longju)):
            if Longju[i] <= int(0.9 / xiangyuan) or Longju[i] >= int(1.5 / xiangyuan):  # ( 0.9/xy <= longju <= 1.5/xy)
                continue
            else:
    ...
    def Warning(self,K,contours,xiangyuan,out_name1,image):
        ...
        for i in range(len(contours)):
            ...
            #cv2.putText(image, str(i) + '', (int(x), int(y)), cv2.FONT_HERSHEY_PLAIN, 5, (147, 20, 255), 5)
            if long_h >= (2.0/xiangyuan) and long_w >=(2.0/xiangyuan): #由于正常陇的宽度大约为0.8米，地膜间距约为0.6米，所以连陇阈值取2.0米
                ...
                cv2.putText(image, str(i)+'Error_long !', (int(x),int(y)),cv2.FONT_HERSHEY_PLAIN, 5, (147, 20, 255), 5)
            else:
                if abs(K[i]-K_mean) > 15 and abs(K[i]-K_mean) < 30: #斜率阈值0.45 误差为24°，陇距误差0.1米，误差8%
                    ...
                    cv2.putText(image,str(i)+'Error_K!', (int(x), int(y)), cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 5)
...
if __name__ == '__main__':
    path = './test/miaozipo/yiqi'
    dk_list=os.listdir(path)
    for dk in dk_list:
        dk_path=os.path.join(path,dk)
        tk_list=os.listdir(dk_path)
        tk_mask_path=os.path.join(dk_path,'tk_masks')
        masks_list=os.listdir(tk_mask_path)
        tk_tif_path=os.path.join(dk_path,'tk_tifs')
        tifs_list = os.listdir(tk_tif_path)
        for img in tifs_list:
            tif_name = os.path.join(tk_tif_path,img)
            tk_name=img[:-4]
            mask_name=os.path.join(tk_mask_path,tk_name+'_mask.png')
            if not os.path.exists(os.path.join(dk_path,'tk_json')):
                os.makedirs(os.path.join(dk_path,'tk_json'))
            save_json_path = os.path.join(dk_path,'tk_json',tk_name +'.json')
            out_name1 =os.path.join(tk_mask_path, tk_name+'_err.png')
            if not os.path.exists(os.path.join(dk_path,'tk_visual')):
                os.makedirs(os.path.join(dk_path,'tk_visual'))
            out_name2 = os.path.join(dk_path, 'tk_visual', tk_name +'.png')
            ...
            #     print('big image!')
            #     continue
            hangju = int(2.0 / xiangyuan)  # line distance

            # 求出切分线
            contours,contours_warn,K_cal= C.contoursdetect(mask_name)   # 输出轮廓
            #预警
            err_K,err_long=C.Warning(K_cal,contours_warn,xiangyuan,out_name1,img) #判断连陇
            if len(err_long) != 0 or len(err_K) != 0:
                ...
                # 剔除异常
                Center_Points, Longju = C.error_points(Center_Points, Longju, xiangyuan)
                # 陇的绑定
