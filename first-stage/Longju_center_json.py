# -*- coding:utf-8 -*-
#读取cvat的地膜陇距结果变换成中心点陇距然后绑定陇序保存json
import numpy as np
import cv2
import time
import os
...
class Cal():
    ...
    def error_points(self,PP,Longju):
        ...
        for pp_c,longju in zip(PP,Longju):
            ...
            for i in range(len(longju)):
                if longju[i] <= 50 or longju[i] >= 300:
                    continue
                else:
                    pp_.append(pp_c[i])
    ...
    def linedetect(self,png_name,img_name,out_name2,hangju):
        ...
        print('----------切分线构造完成----------')
        return contours,A,H,W,k

    def visual(self,png_name,PP,A):
...
if __name__ == '__main__':
    path='E:/pytorch-Unet/Pytorch-UNet-master/test/zhaojiaba/yiqi/dk_4/'
    for num in range(7,7+1):
        ...
        C = Cal()
        start = time.time()
        contours,A,H,W,k=C.linedetect(png_name,img_name, out_name2,hangju)
        longdui=C.read_xml(cvatxml_path) #读取cvat核查结果 输出陇对的点集
        centerlongdui=C.findcenterpoints(longdui,A,k) #将两陇地膜边界距离坐标变换成陇中心点的距离
        #求陇距
        PP, Longju = C.line_contours(contours, centerlongdui)
        PP,Longju=C.error_points(PP,Longju)
        C.visual(png_name,PP,A)
