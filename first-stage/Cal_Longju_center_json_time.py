...
class Cal_Longju():
    ...
    def contoursdetect(self, mask_name):
        ...
        #matRotate = cv2.getRotationMatrix2D((W * 0.5, H * 0.5), 180, 1)
        #dst = cv2.warpAffine(dst, matRotate, (W, H))
        dst = dst[:, :, 0]
    ...
    def linedetect(self, img, K_cal, hangju):
        """
        # out_name:可视化
        # hangju:自拟定的切分线间隔
        """
    ...
    def small_contours(self,mask_name,H,W,trend,out_name):
        img=cv2.imread(mask_name)
        img=img[:,:,0]
        img0= np.ones((H,W),dtype=np.uint8)
        img1=cv2.drawContours(img0, trend, -1, (255, 255, 255), 21) #10 → 1
        img2=cv2.bitwise_and(img,img1)
        #cv2.imwrite(out_name, img2)
        ret, binary = cv2.threshold(img2, 127, 255, cv2.THRESH_BINARY)
        contours_small, hierarchy_samll = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) #NONE → SIMPLE
        print('----------小轮廓完成----------')
        return contours_small,img2

    def order_points(self,pts):
        rect = np.zeros((4, 2), dtype="int32")
        su = pts.sum(axis=1)
        rect[0] = pts[np.argmin(su)]  # 左上角的点具有最小的和
        rect[2] = pts[np.argmax(su)]  # 右下角的点具有最大的和
        di = np.diff(pts, axis=1)
        rect[1] = pts[np.argmin(di)]  # 右上角的点具有最小的差值
        rect[3] = pts[np.argmax(di)]  # 左下角的点具有最大的差值
        # 返回排序坐标(依次为左上右上右下左下)
        return rect

    def short_contours(self,contours_small,img2,out_name):
        contours_short=[]
        for c in contours_small:
            c = c.reshape(-1, 2)
            rect = cv2.minAreaRect(c)
            #print(rect)
            p0 = np.where(c == rect[0])[0]
            index_0 = [p0[i] for i in range(len(p0) - 1) if p0[i] == p0[i + 1]][0]
            p1 = np.where(c == rect[1])[0]
            index_1 = [p1[i] for i in range(len(p1) - 1) if p1[i] == p1[i + 1]][0]
            p2 = np.where(c == rect[2])[0]
            index_2 = [p2[i] for i in range(len(p2) - 1) if p2[i] == p2[i + 1]][0]
            p3 = np.where(c == rect[3])[0]
            index_3 = [p3[i] for i in range(len(p3) - 1) if p3[i] == p3[i + 1]][0]
            c_inex=sorted([index_0,index_1,index_2,index_3])
            c0 = np.vstack((c[0:c_inex[0],:],c[c_inex[3]:,:]))
            c1 = c[c_inex[0]:c_inex[1],:]
            c2 = c[c_inex[1]:c_inex[2],:]
            c3 = c[c_inex[2]:c_inex[3],:]
            c_ = np.argsort([c0.size,c1.size,c2.size,c3.size])
            c_new = np.vstack(([c0,c1,c2,c3][c_[0]],[c0,c1,c2,c3][c_[1]]))
            if len(c_new) != 0:
                c_new_ = c_new.reshape(-1, 1, 2)
                contours_short.append(c_new_)
        cv2.drawContours(img2, contours_small, -1, (255, 255, 255), 3)
        cv2.imwrite(out_name, img2)
        print('----------小轮廓优化完成----------')
        return contours_short
    ...
    def error_points(self, Center_Points, Longju, xiangyuan, longju_gt):
        ...
        for i in range(len(Longju)):
            if Longju[i] <= int(longju_gt*0.5 / xiangyuan) or Longju[i] >= int(longju_gt*1.5 / xiangyuan):  # ( 1.2-0.6/xy <= longju <= 1.2+0.6/xy)
                continue
            else:
...
if __name__ == '__main__':
    ...
    for num in range(1, 1+ 1):
        ...
        out_name3 = path + 'tk_visual/tk_' + str(num) + '_.png'

        start = time.time()
        print('----------开始处理：', tif_name)
        C = Cal_Longju()
        img, xiangyuan, H, W = C.readTif(tif_name)
        #matRotate = cv2.getRotationMatrix2D((W * 0.5, H * 0.5), 180, 1) #翻转测试
        ...
        if len(err_long) > 1 or len(err_K) > 0:
            ...
            # 求出切分线
            qiefen_Points, trend= C.linedetect(img, K_cal, hangju)  # 输出切分线的点集
            contours_small,img2 = C.small_contours(mask_name,H,W, trend, out_name3)
            contours_short = C.short_contours(contours_small,img,out_name3)

            # 计算陇距
            Center_Points, Longju = C.line_contours(contours_short, qiefen_Points)  # 输出交点坐标，陇距
            # 剔除异常
