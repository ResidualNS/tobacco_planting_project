...
def train_net(net, device, epochs=5, batch_size=8, lr=0.001, val_percent=0.1, save_cp=True, img_scale=0.5):
    ...
    for epoch in range(epochs):
        ...
        #     if val_score > best_val_score:
        #         best_val_score = val_score
        #         torch.save(chkpt,  dir_checkpoint + 'unet_chkpt_%d.pth' % (epoch+1))
...
def get_args():
    ...
    parser.add_argument('-b', '--batch-size', metavar='B', type=int, nargs='?', default=16, help='Batch size', dest='batchsize')
    ...
    #parser.add_argument('--weights', type=str, default='./checkpoints/0730.pth', help='initial weights path')
